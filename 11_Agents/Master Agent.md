...
The commander. One brain that routes work to lane agents, keeps run state, assembles the approval board, and sends exactly one push to Dillon per cycle. **Runtime skill:** `/dillon-command` (`.claude/skills/dillon-command/SKILL.md`). Full operational spec: `11_Agents/64gb Morning Orchestrator Spec 2026-07-08.md`. Any model can run this role; the contract is markdown + JSON, not a model feature.
...
- Run artifacts go to `automation-runs/dillon-command/YYYY-MM-DD/` (profile: `_os/automation/profiles/dillon-command.json`)
...
- Eight parallel lanes: comms, clients, intelligence, websites, outreach, ads, reporting, command — see `12_Brain/concepts/Dillon Command Center.md`
