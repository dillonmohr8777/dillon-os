...
2. `11_Agents/Rockbot Operating System/evidence/codex-rollouts/*.md` — latest Codex rollouts
3. `09_Transcripts/` if any new files since last run
4. Repo git log last 7 days for session-related commits

## Actions

1. List session files modified in last 7 days.
2. Extract **unfinished** items: "Next Steps", "TODO", "blocked", open checkboxes.
3. For each item tied to a client name, append a dated bullet under that client's notes (section `## Session sync`).
