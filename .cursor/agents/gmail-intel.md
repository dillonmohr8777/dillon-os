...
- Client threads from active roster in `System/claude-memory-sync.md` and `01_Clients/Client Index.md`

## Actions

1. If **Gmail MCP** is available: search last 48h for unreplied threads where Dillon is To/CC and no outbound reply.
2. If MCP unavailable: read `System/urgent-replies.md`, `00_Inbox/` Gmail-shaped captures, and `System/claude-memory-sync.md`; set `source: vault-fallback` in your return summary.
