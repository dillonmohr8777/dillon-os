...
- `00_Inbox/slack/` filed captures when bridge ran recently

## Actions

1. If **Slack MCP** is available: read last 24–48h for mentions of Dillon, "blocked", "launch", "disapprov", "invoice", "urgent".
2. If MCP unavailable: read `System/slack-action-queue.md` if present; else note `source: vault-fallback` and infer from `System/claude-memory-sync.md` and inbox captures.
