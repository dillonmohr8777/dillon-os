...
.slug-${slug} .hero h1,.slug-${slug} .section-head h2,.slug-${slug} .story h2,.slug-${slug} .feature h2,.slug-${slug} .spotlight h2,.slug-${slug} .contact-intro h2,.slug-${slug} .closing h2,.slug-${slug} .catalog-card h3,.slug-${slug} .footer-identity strong{font-family:var(--display);font-weight:700}
...
.slug-${slug} .site-header{background:color-mix(in srgb,var(--paper) 55%,transparent);border-bottom-color:color-mix(in srgb,var(--ink) 10%,transparent);box-shadow:0 10px 24px rgba(0,0,0,.12)}
.slug-${slug} .hero-media figure{border:1px solid color-mix(in srgb,#fff 45%,transparent);box-shadow:0 22px 32px rgba(0,0,0,.22),inset 0 1px 0 color-mix(in srgb,#fff 55%,transparent);border-radius:calc(var(--radius) + 8px)}
.slug-${slug} .offering-card,.slug-${slug} .experience-grid article,.slug-${slug} .catalog-card,.slug-${slug} .contact-card{background:color-mix(in srgb,var(--paper) 38%,transparent);border:1px solid color-mix(in srgb,#fff 35%,var(--ink) 12%);box-shadow:0 14px 26px rgba(0,0,0,.16);backdrop-filter:blur(24px) saturate(170%)}
...
.slug-${slug} .hero h1{font-weight:700;letter-spacing:-.02em;max-width:14ch}
...
.slug-${slug} .surface-accent .section-head h2 mark{color:var(--on-accent);box-shadow:inset 0 -.12em 0 var(--accent2)}
.slug-${slug} .surface-deep .section-head h2 mark{color:var(--accent2);box-shadow:inset 0 -.12em 0 color-mix(in srgb,var(--accent2) 35%,transparent)}
`,
    brutal: `
...
.slug-${slug} .hero h1{letter-spacing:-.01em}
.slug-${slug} .hero-media figure,.slug-${slug} .gallery-grid figure{border-radius:4px;filter:contrast(1.05) saturate(.92)}
.slug-${slug} .proof-grid{background:color-mix(in srgb,currentColor 4%,transparent)}
...
.slug-${slug} .hero h1 mark{background:transparent;color:var(--accent2)}
.slug-${slug} .hero-media figure{border:2px solid color-mix(in srgb,var(--accent2) 55%,transparent);box-shadow:0 0 0 1px color-mix(in srgb,var(--accent) 40%,transparent),0 20px 30px rgba(0,0,0,.22)}
