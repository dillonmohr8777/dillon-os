...
.slug-${slug} .site-header{background:color-mix(in srgb,var(--paper) 55%,transparent);border-bottom-color:color-mix(in srgb,var(--ink) 10%,transparent);box-shadow:0 8px 22px color-mix(in srgb,var(--deep) 12%,transparent)}
.slug-${slug} .hero-media figure{border:1px solid color-mix(in srgb,#fff 45%,transparent);box-shadow:0 12px 28px color-mix(in srgb,var(--deep) 28%,transparent),inset 0 1px 0 color-mix(in srgb,#fff 55%,transparent);border-radius:calc(var(--radius) + 8px)}
.slug-${slug} .offering-card,.slug-${slug} .experience-grid article,.slug-${slug} .catalog-card,.slug-${slug} .contact-card{background:color-mix(in srgb,var(--paper) 38%,transparent);border:1px solid color-mix(in srgb,#fff 35%,var(--ink) 12%);box-shadow:0 10px 24px color-mix(in srgb,var(--deep) 14%,transparent);backdrop-filter:blur(24px) saturate(170%)}
...
.slug-${slug} .hero h1{font-weight:500;letter-spacing:-.02em;max-width:14ch}
...
.slug-${slug} .marquee-strip{border-block:4px solid var(--ink)}
`,
    warm: `
...
.slug-${slug} .hero h1{letter-spacing:-.02em}
.slug-${slug} .hero-media figure,.slug-${slug} .gallery-grid figure{border-radius:4px;filter:contrast(1.05) saturate(.92)}
.slug-${slug} .proof-grid{background:linear-gradient(135deg,color-mix(in srgb,currentColor 6%,transparent),transparent)}
...
.slug-${slug} .hero-media figure{border:1px solid color-mix(in srgb,var(--accent2) 55%,transparent);box-shadow:0 0 0 1px color-mix(in srgb,var(--accent) 40%,transparent),0 12px 28px color-mix(in srgb,var(--accent) 35%,transparent)}
