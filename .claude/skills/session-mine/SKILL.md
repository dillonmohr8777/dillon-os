---
name: session-mine
description: Mine the session that just happened for decisions, mistakes, and confirmed patterns — write them into 12_Brain/raw/sessions/ as a dated note so work already done becomes memory. Run at the end of any session that decided something.
---
...
2. Write `12_Brain/raw/sessions/YYYY-MM-DD - <short-topic>.md` (append a counter if it
   exists). Bullet points, each self-contained, no narrative padding. This is
   a raw capture: once written it is read-only history.
3. Append one line to `12_Brain/raw/sessions/session-log.md`:
   `- YYYY-MM-DD — <topic> (mined)`.
4. Do **not** compile into 12_Brain/entities/concepts here — that's `/vault-compile`'s
