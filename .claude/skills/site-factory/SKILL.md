...
3. **Write the brief.** Copy the matching starter from `_templates/site-factory/starters/` — HVAC, landscape/concrete, bridal, painting, wellness, or local ads. Resolve the path with `node _templates/site-factory/pick-starter.js "<vertical or attitude>"`. If nothing matches, copy `_templates/site-factory/example-brief.json`. Fill every section with real, verified copy. Never ship a starter's 555 phone or demo address. Voice rules from `System/writing-rules.md`: short declarative hero claim, proof strip states facts not adjectives, offerings name real items, no em dashes, contractions.
4. **Build.**
