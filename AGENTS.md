...
|---|---|---|---|---|
| D.I.L.L.O.N. OS (HUD) — flagship | `_os/` | `node _os/server.js` | http://127.0.0.1:4242 | Reads the vault + `12_Brain` live. `GET /api/state` includes `brain` and `outreach` vitals. Serves `/outreach/<batch>/`. Phone PWA: https://dillon-os-hud.netlify.app (read-only snapshot; Command Deck stays desktop). |
...
```
node --test _os/test/brain-hud.test.js _os/test/public-safety.test.js _os/test/hud-mobile.test.js _os/outreach-engine/tests/outreach-engine.test.js
```
