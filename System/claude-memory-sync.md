...
Single source of truth for all Claude instances across Dillon OS. Updated by **`competitive-task-orchestrator`** daily at 1:00 PM ET via `memory-consolidator` subagent. Legacy `vault-integrity-sync` and `chat-to-vault-sync` crons are retired.
