...
| Site health | `_os/automation/bin/site-health.js` | `websites` |
| Prospect radar | `Daily-Briefs/radar-*.md`, `12_Brain/state/radar/` | `outreach` |
| Cross-instance memory | `System/claude-memory-sync.md` | `memory-consolidator` |

## Umbrella workflow

One CLI scaffolds each run under `automation-runs/competitive-task-orchestrator/YYYY-MM-DD/`:

```bash
node _os/automation/bin/competitive-task-orchestrator.js --preflight --date YYYY-MM-DD
node _os/automation/bin/competitive-task-orchestrator.js --agent-mode --date YYYY-MM-DD
```

Subagent definitions: `.cursor/agents/*.md`. Skill: `.claude/skills/competitive-task-orchestrator/SKILL.md`.
