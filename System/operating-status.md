...
- Nexla is a new Momentum 360 Google Ads engagement and is **not** in the 14-client count until the canonical registry includes the route. Live check 2026-08-19T21:40Z: Read-only invite for CID `791-780-2207` is still UNREAD and starred; Jayashree reply is still a Gmail draft on the onboarding thread, not sent. MCC to send from is Momentum Ads Manager `743-802-1996` (not the Hermes Agent MCC). This environment has no signed-in Ads UI and cannot send that manager-link via the connected Ads OAuth. See [[12_Brain/05_Projects/2026-08-19 - Nexla Google Ads onboarding]].
