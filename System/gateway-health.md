...
last_updated: 2026-09-15T18:31:51.012Z
gateway_pid: 18656
state: "stale-warn: process alive and command identity verified; root running; Telegram connected; heartbeat and log silent for over 4.5 hours after benign housekeeping"
heartbeat_age_sec: 16540
conflicts_1h: 0
conflicts_6h: 0
conflicts_24h: 0
source_refs:
...
## 2026-09-15T18:31:51.012Z - STALE-WARN 18656 conflicts 0/0/0

- **Gateway PID:** 18656.
- **Process liveness:** alive; native Windows process query identifies `python.exe` (working set 95,252,480 bytes) running `hermes.exe gateway run`, matching the intended gateway command.
...
- **Heartbeat age:** 16,540 seconds (4h 35m 40s). Telegram sub-state age is also 16,540 seconds.
...
- **Log freshness:** final event and file mtime are 16,538 seconds old (4h 35m 38s). The final event was benign `Gateway housekeeping started`; there is no current polling conflict in the required tail.
...
- **State:** **STALE-WARN** — the intended process is alive and correctly identified, and Telegram retains connected state, but the root heartbeat and gateway log have remained frozen across repeated snapshots for over 4.5 hours after benign housekeeping.
