# Google Ads Optimization Queue

## High Priority

- [ ] **Bar Crawl USA** — Resolve 2 disapproved ads (Halloween / Fall Cocktail Crawl language); confirm Soulard PMax pacing ~$15–20/day. Account `435-710-2897`. (~105 days overdue)
- [ ] **NKCDC** — Launch ready but blocked on client Free Tax Prep landing page; nudge Anthony. (~105 days)
- [ ] **LinkEZE** — Fix enhanced conversions diagnostics; confirm MFA on `809-600-6448`. (~115 days overdue)

## Medium Priority

- [ ] **Fresh Blends / Replenish** — Confirm 2026-04-13 campaign launch pacing; send first-week snapshot to Mia.
- [ ] **Kimberly James Bridal** — Publish Timeline page; verify GA4/GSC indexing per Mac.

## Low Priority

- [ ] **Shadow HVAC** — Confirm LSA background check cleared; send Mike catch-up report.
- [ ] **Omega Landscaping** — Confirm David/John Belaska account review meeting.

## Completed

