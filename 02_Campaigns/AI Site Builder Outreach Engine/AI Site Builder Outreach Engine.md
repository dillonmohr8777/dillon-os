...
Direct mail vendor selected for test-mode integration: PostGrid. QR automation remains Zapier + QRTiger from a Google Sheet. StackAdapt is retained only as a possible future programmatic-media option.
...
Build through quality gate is automated. Approval stays human. Activation uses QR via Zapier/QRTiger and PostGrid for mail. The local direct-mail plan is implemented; external test calls still need an approved PostGrid account, and public deployment needs the exact verified Netlify target for each batch.

**Current external gates as of 2026-07-30:** live Indeed intake needs Partner API entitlement; PostGrid test calls need an approved account and secret locator; public deployment needs an exact verified Netlify target. QR path is ready once the sheet zap is wired to `manifest.csv`. Outreach, spending, publishing, and account changes require explicit approval.
...
- Dillon self-reported roughly 50 to 55 prospect sites in the pipeline, with mobile optimization and integrations still incomplete.
- The count is an estimate, not a verified inventory or a send-ready total.
- Next action: deterministically count unique sites, map ownership, test mobile and conversion paths, and report verified-ready, blocked, and duplicate totals before outreach.
- Source: [Slack thread](https://momentum3d.slack.com/archives/C04HXSVN2CS/p1785507053296869)
