...
| 2.4–5.0 | **Line 2** | Hard cut that changes the axis as well as the size: flush-left at the 120px margin, both lines low in the frame, `build momentum.` running nearly to the right edge. `Great ones` + `build momentum.`, the accent on `momentum`. Beat 1's centred mid-frame line and this one must not be describable by the same sentence. |
| 4.8–8.4 | **The stack** | Type shrinks and pins to the upper third. Below it, three device cards — a phone, a browser, a map card — fly in on separate arcs with real perspective (`rotateX/rotateY`, a shared vanishing point) and settle into a loose, deliberately *un*-gridded arrangement that **crops off three frame edges** — browser off the left, phone off the top and right, map off the bottom. Cards fully inside the frame with even margins read timid. The pinned title goes flush-left, not centred. Caption under: `Illustrative UI — not client work.` |
...
| 24.2–26.2 | **The dart** | Everything clears. One object: the Momentum mark travels a continuously rising arc from lower-left to the lockup's mark position, growing to ~300px so it owns the frame, leaving a short signal comet that dissipates behind it. The arc must never reverse direction, and the trail must not taper into a shape that reads as a map pin — beats 3 and 5 already spend that shape. It lands, then settles to lockup size while the wordmark wipes in beside it: one hard handoff, no cross-dissolve. |
| 26.0–30.0 | **Lockup** | Mark + `MOMENTUM` set together, centred. Under it, one line: `Philadelphia. Since 2015.` Under that, `needmomentum.com`. Hold two full seconds — the last frame is the frame people screenshot. |
