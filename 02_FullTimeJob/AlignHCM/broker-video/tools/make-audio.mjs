...
const BPM = 122, BEAT = 60 / BPM, BAR = BEAT * 4;
...
for (let i = 0; i < N; i++) {
  ...
  // sidechain pump: deep duck after every beat while any kick is active
  ...
  // ---- kick: click + pitch-swept body, full and intro (filtered) variants ----
  let kick = 0;
  ...
  if (kEnv > .001) {
    ...
    // intro: muffled club-through-the-wall kick
    lpIntro += .012 * (body - lpIntro);
    kick += lpIntro * 34 * .28 * kickIntroOn(t) * (i % 2 ? 1 : 1); // lp gain comp
  }
  ...
  // ---- bass: sine sub + quiet saw layer for presence, 8th gated ----
  const rootF = ch[0] / 2;
...
// ---- master: normalize, drive into soft limiter for EDM loudness ----
let peak = 0;
for (let i = 0; i < N; i++) peak = Math.max(peak, Math.abs(L[i]), Math.abs(R[i]));
const drive = 5.5 / peak;                 // push into the limiter
const ceil = Math.pow(10, -1.2 / 20);     // -1.2 dBFS ceiling
