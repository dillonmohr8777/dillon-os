...
type ProspectBrand = Pick<Prospect, 'name' | 'seed'> & {
  logo?: string
  wordmark?: string
}

const alignBrand: ProspectBrand = {
  name: 'Align HCM',
  logo: '/clients/align-hcm.png',
  seed: 242825734,
}

const audienceBrand: ProspectBrand = {
  name: 'You',
  wordmark: 'YOU',
  seed: 8777242,
}
...
async function buildProspectCloud(brand: ProspectBrand, count: number, compact: boolean): Promise<PointCloud> {
  ...
  if (brand.wordmark) {
    await document.fonts.load('850 240px "Unbounded Variable"')
    let fontSize = 240
    context.font = `850 ${fontSize}px "Unbounded Variable", sans-serif`
    const availableWidth = canvas.width - 64
    const measuredWidth = context.measureText(brand.wordmark).width
    ...
    context.lineWidth = Math.max(5, fontSize * .026)
    context.strokeText(brand.wordmark, canvas.width / 2, canvas.height / 2 + fontSize * .035)
    context.fillStyle = '#f4f7fb'
    context.fillText(brand.wordmark, canvas.width / 2, canvas.height / 2 + fontSize * .035)
  } else {
    if (!brand.logo) throw new Error(`${brand.name} logo source is missing.`)
    const image = new Image()
    image.decoding = 'async'
  ...
  if (!candidates.length) throw new Error(`${brand.name} logo alpha mask is empty.`)

  const random = seededRandom(brand.seed)
  const scatter = new Float32Array(count * 3)
  const target = new Float32Array(count * 3)
  const colors = new Float32Array(count * 3)
  const phases = new Float32Array(count)
  ...
  const rawTargetHeight = targetWidth * (maskHeight / maskWidth)
  const maxTargetHeight = compact ? 3.4 : 3.7
  const logoScale = rawTargetHeight > maxTargetHeight ? maxTargetHeight / rawTargetHeight : 1
  const fittedWidth = targetWidth * logoScale
  const targetHeight = rawTargetHeight * logoScale
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const candidateIndex = candidates.length >= count
      ...
      : index % candidates.length
    const sample = candidates[candidateIndex]
    const tx = ((sample[0] - minX) / maskWidth - 0.5) * fittedWidth + (random() - 0.5) * 0.003
    ...
    if (luminance < 0.18) color.lerp(new THREE.Color('#eef5ff'), 0.82)
    colors.set([color.r, color.g, color.b], index * 3)
    phases[index] = random()
  }
  return { scatter, target, colors, phases }
...
export function ParticleLogo() {
  ...
  return (
    <div className={`particle-logo${reducedMotion ? ' particle-logo--static' : ''}`} role="img" aria-label="Complete IMMOHRTAL logo rendered as particles">
...
function ProspectParticleLogo({ brand, compact }: { brand: ProspectBrand; compact: boolean }) {
  return (
    <div className="prospect-particle-logo" role="img" aria-label={`${brand.name} logo rendered as particles`}>
...
export function ProspectSequence({ prospect }: { prospect: Prospect | null }) {
  const [stage, setStage] = useState(0)
  const compact = useMediaQuery('(max-width: 760px)')
  const reducedMotion = useMediaQuery('(prefers-reduced-motion: reduce)')
  const targetBrand = prospect ?? audienceBrand
  const prospectStages = useMemo(() => [
    ...
    { kind: 'logo', value: 'target', duration: 5000 },
  ...
  if (reducedMotion) {
    return (
      <div className="prospect-sequence__reduced" aria-label={`I did this for Align HCM. I can do it for ${prospect?.name ?? 'you'}.`}>
        <span>I did this for</span><img src="/clients/align-hcm.png" alt="Align HCM" />
        <span>I can do it for</span>
        {prospect
          ? <img src={prospect.logo} alt={prospect.name} />
          : <strong className="prospect-sequence__you">YOU</strong>}
      </div>
    )
  }
  ...
  return (
    <div className="prospect-sequence__stage" aria-live="polite">
      ...
      {current.kind === 'logo' && (
        <ProspectParticleLogo
          key={`${stage}-${current.value}-${prospect?.slug ?? 'general'}`}
          brand={current.value === 'align' ? alignBrand : targetBrand}
          compact={compact}
        />
      )}
    </div>
