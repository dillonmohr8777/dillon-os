...
5. `--apply --confirm-apply` needs Dillon's instruction plus a Jason/Momentum token. Verify portal 50612503 before every write. Merge onto existing filter trees; do not replace URL/form branches. `--workflows` creates Source-copy and organic-notify flows. `--remaining` creates `utm_*` form-field properties; it cannot edit Zapier, WordPress, or GTM.
6. Draft Slack in the vault. Post only when Dillon says send or fix all.
