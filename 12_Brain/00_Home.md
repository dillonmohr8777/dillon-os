---
note_type: dashboard
status: active
owner: Dillon Mohr
updated: 2026-09-02
review_cadence: daily
tags:
  - brain
  - dashboard
---

# Brain Home

> [!important] Connected navigation
> Open [[10_Maps/00_Atlas|Dillon OS Knowledge Atlas]] to move through clients,
> growth, agents, decisions, sessions, standards, and personal ventures as one
> connected system.

Think, capture, decide, and execute from one place. Client truth remains in
`01_Clients/`; this layer connects that truth to projects, decisions, research,
and reusable memory.

> [!tip] Quick capture
> Put unsorted material in [[00_Inbox/Start Here|00_Inbox]]. Use the Brain
> Capture template when the source needs a durable receipt.

## Active projects

![[Bases/Command Center.base#Active Projects]]

## Operating priorities

- [[05_Projects/2026-08-24 - Perfect outcome graph engineering|Perfect outcome graph engineering]]
- [[05_Projects/2026-07-30 - Dillon OS five-goal operating plan|Five-goal operating plan]]
- [[05_Projects/2026-07-30 - Qualified pipeline recovery|Qualified pipeline recovery]]
- [[04_Decisions/2026-07-30 - Marketing Chief is Dillon's sole agent interface|Marketing Chief interface decision]]
- [[04_Decisions/2026-09-01 - Route numeric forecasts to a specialist|Forecast specialist routing]]
- [[04_Decisions/2026-09-02 - Separate deliverable prediction from workload forecasting|Predictive work-planner decision]]
- [[05_Projects/Experiments/EXP-TIMESFM-FORECAST-ROUTER|TimesFM forecast-router experiment]]
- [[04_Decisions/2026-09-01 - Fable 5.1 routing and effort defaults|Fable 5.1 routing and effort defaults]]
- [[06_Research/2026-09-01 - Fable 5.1 launch signal sweep|Fable 5.1 launch signal sweep]]

## Strategy brain

![[Bases/Strategy Library.base#Strategy Library]]

- [[03_Concepts/Marketing Intelligence Operating System|Marketing intelligence operating system]]
- [[03_Concepts/AEO GEO and AI Discovery|AEO, GEO, and AI discovery]]
- [[03_Concepts/Automation and Workflow Engineering|Automation and workflow engineering]]
- [[03_Concepts/Specialist Forecast Router|Specialist forecast router]]
- [[03_Concepts/Predictive Work Planner|Predictive work planner]]
- [[03_Concepts/Leading Indicators|Leading indicators]]
- [[09_Ops/Knowledge Coverage|Knowledge coverage audit]]

## Experiment queue

![[bases/Experiment Queue.base#Active Experiments]]

## Decisions to review

![[Bases/Command Center.base#Decision Review]]

## Research expiring

![[Bases/Command Center.base#Research Due]]

## Memory needing review

![[Bases/Memory Ledger.base#Needs Review]]

## Client intelligence

![[Bases/Client Intelligence.base#Active Clients]]

## Portfolio strategy overlays

![[Bases/Client Strategy Overlays.base#Immediate Attention]]

- [[09_Ops/Client Intelligence Coverage|Client intelligence coverage]]
- [[../01_Clients/Client Index|Canonical client and relationship index]]

## Weekly and monthly reports

![[Bases/Report Archive.base#All Reports]]

- [[07_Reviews/Reports/README|Report Archive]]
- [[../04_SOPs/Weekly and Monthly Report Brain Ingestion|Report ingestion SOP]]

## Daily loop

![[Bases/Daily Communication Intelligence.base#Immediate Attention]]

1. Capture without filing friction.
2. Review the Gmail and Slack intelligence row for new decisions, commitments,
   blockers, deliverables, and unresolved routing.
3. Pick one finite project outcome.
4. Record decisions with rationale and evidence.
5. Let agents compile durable lessons and corrections.
6. End the day with a short review; end the week with synthesis.

## Operations

- [[10_Maps/00_Atlas|Knowledge Atlas]]
- [[10_Maps/README|Map Maintenance]]

- [[09_Ops/Architecture|Architecture]]
- [[09_Ops/Claude Target Architecture Proposal|Claude Target Architecture Proposal]]
- [[09_Ops/AGENT_PROTOCOL|Agent Protocol]]
- [[09_Ops/Runbook|Runbook]]
- [[09_Ops/Health|Health]]
- [[09_Ops/Knowledge Coverage|Knowledge Coverage]]
- [[09_Ops/Client Intelligence Coverage|Client Intelligence Coverage]]
- [[Bases/Daily Communication Intelligence.base|Daily Communication Intelligence]]
- [[../INDEX|Vault Index]]
