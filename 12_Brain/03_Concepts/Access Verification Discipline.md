...
- Unread Gmail is not the only accept signal, but it is a live one. A cheaper accept-probe than `listAccessibleCustomers` (which is quota-fragile on basic-access developer tokens) is GAQL `SELECT customer.id, customer.descriptive_name FROM customer` on the invited CID. After a direct Users-tab accept, that query succeeds without `login-customer-id`. `USER_PERMISSION_DENIED` means the user invite is not in effect yet (proven on Nexla `791-780-2207`, 2026-08-19T21:48Z). Do not treat Composio `proxy_execute` as a CustomerClientLink sender — it is not that mutate surface.
