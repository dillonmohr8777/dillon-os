...
2. **Preserve existing drafts**: before drafting, search the live thread and `list_drafts`; update the matching draft in place; create new only if no match. (Avoided duplicate drafts for Omega/David, Mike/Revive, Ambika.) **Exception:** Gmail `update_draft` on a threaded reply can move it onto a new thread (Nexla, 2026-08-19, twice). Trash the unthreaded draft and recreate with `create_draft` plus `replyToMessageId`. Confirm `threadId` before leaving it.
...
## Links
- [[12_Brain/03_Concepts/Evidence Boundaries in Reporting|Evidence Boundaries in Reporting]] · [[12_Brain/03_Concepts/Access Verification Discipline|Access Verification Discipline]] · [[12_Brain/03_Concepts/Truth Hierarchy|Truth Hierarchy]]
