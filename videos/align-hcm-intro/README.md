# Align HCM · Brand Intro (48.4s)

A 48 second animated brand introduction for Align HCM. Deep navy stage, orange
...
inside a pink speech bubble, and a global key punches it straight out. Edge
...
The flood fill alone keeps *every* enclosed white region, and those come in two
kinds. White ink inside a brand shape (the "Hi") is artwork and must survive;
the counters of a glyph (the holes in the B, o and b of "Bob") are background
and must go, or each one fills in as a solid white blob on navy. What separates
them is what they touch, so each enclosed region is measured against the ring of
pixels around it and cut only when that ring is neutral rather than saturated.
...
The strip is UKG, Dayforce, HiBob and Paylocity. Workday and ADP are still
fetched and processed into `assets/logos/` so the artwork is on hand, but they
are not marks this film shows: the claim is one partner across *many* platforms,
not every platform in the market. Platform logos are scaled to equal
**optical area**, not equal width or
...
| 6 | 20.6 | 25.6 | 5.0 | Platform logos: UKG, Dayforce, HiBob, Paylocity. |
...
| 11 | 42.6 | 48.4 | 5.8 | End card: lockup, tagline, alignhcm.com. |
...
# 3. render
node render.mjs --jobs 3         # about 10 minutes for the full 48.4s
```
