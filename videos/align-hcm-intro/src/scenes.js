...
/* Platform marks sit in light cards so each one can wear its own colours: UKG
   ...
   same optical area instead, with a per mark weight for ink density: Dayforce
   is airy lowercase and wants a little more, Paylocity is a stacked mark over
   a wordmark and reads dense at the same area, so it wants less. The card's
   own padding sets the clear space, and max-width/max-height on the image is
   the hard guarantee that nothing can ever crowd or overrun its box. */
const LOGO_AREA = 23500;
const PLATFORMS = [
  { key: 'ukg',       name: 'UKG',       aspect: 483 / 186, weight: 1.00 },
  { key: 'dayforce',  name: 'Dayforce',  aspect: 900 / 214, weight: 1.07 },
  { key: 'hibob',     name: 'HiBob',     aspect: 432 / 176, weight: 1.00 },
  { key: 'paylocity', name: 'Paylocity', aspect: 302 / 295, weight: 0.80 },
];
...
const SCENES = [
  ...
  /* 6. platform grid --------------------------------------------------- */
  {
    ...
    html: `
      <div class="pad" style="left:130px;right:130px">
        ${eyebrow('One partner, many platforms')}
  ...
  /* 11. end card, rebuilt from the supplied still ------------------------ */
  {
    id: 's11', in: 42.6, out: 48.4,
    html: `
      <div class="endcard">
        <div data-lock>${lockupSVG('l11', 940)}</div>
    ...
    draw(root, lt, dur) {
      drawLockup(root, 'l11', seg(lt, 0.1, 1.5));
      const lock = q(root, '[data-lock]');
