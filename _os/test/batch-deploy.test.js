...
describe('batch-files collector', () => {
  before(() => {
    ...
    fs.writeFileSync(path.join(tmp, 'compare.html'), NOINDEX);
    ...
    fs.mkdirSync(path.join(tmp, 'sites', 'johnny-s-pizza', 'assets'), { recursive: true });
    fs.writeFileSync(path.join(tmp, 'sites', 'johnny-s-pizza', 'index.html'), NOINDEX);
    fs.writeFileSync(path.join(tmp, 'sites', 'johnny-s-pizza', 'assets', 'logo.png'), 'png');
  });
  ...
  it('ships the hub plus each site path and image assets', () => {
    ...
    assert.deepEqual(slugs, ['andorra-family-dentistry', 'johnny-s-pizza']);
    assert.equal(files.has('/index.html'), true);
    assert.equal(files.has('/compare.html'), true);
    ...
    assert.equal(files.has('/sites/johnny-s-pizza/assets/logo.png'), true);
  ...
  it('reads the Netlify site name from deployBaseUrl', () => {
    assert.equal(
      siteNameFromBatch({ deployBaseUrl: 'https://phl-2026-w33b.netlify.app' }),
      'phl-2026-w33b'
    );
    assert.equal(
