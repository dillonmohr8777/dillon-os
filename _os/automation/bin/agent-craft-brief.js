...
/** Whole days between an ISO day and today, UTC. Negative clamps to 0. */
function ageInDays(isoDay, today = todayISO()) {
  const a = Date.parse(`${isoDay}T00:00:00Z`);
...
function main() {
  ...
  // loadDays() takes the last N *files present*, not the last N calendar days, so a
  // queue nothing has written to since August still yields a full-looking window and
  // a green status. Age the newest receipt against today and say so: a self-audit
  // that reports ok on a dead source teaches the estate that it is healthy.
  const newestReceipt = dayList[span - 1];
  const receiptAgeDays = ageInDays(newestReceipt);
  const stale = receiptAgeDays !== null && receiptAgeDays > span;

  const result = {
    automation_id: 'agent-craft-brief',
    status: stale ? 'stale' : 'ok',
    generated_for: todayISO(),
    newest_receipt: newestReceipt,
  ...
  if (write) {
    ...
    L.push('');
    if (stale) {
      L.push(`> **STALE - do not read these numbers as today.** The newest loop receipt is`);
      L.push(`> \`${newestReceipt}\`, ${receiptAgeDays} day(s) old. Nothing has written`);
      L.push('> `12_Brain/queue/claude-loop-*.jsonl` since, so every figure below describes');
      L.push('> that window and not the current estate. Find out what stopped writing receipts');
      L.push('> before acting on anything here.');
      L.push('');
    }
  ...
  if (stale) {
    process.stderr.write(`agent-craft-brief: STALE - newest receipt ${newestReceipt} is `
      + `${receiptAgeDays} day(s) old; nothing has written `
      + '12_Brain/queue/claude-loop-*.jsonl since.\n');
  }
