...
function loadProfile(profileId) {
  const id = profileId || 'company-os-umbrella';
  return readJson(repoPath('_os/automation/profiles', `${id}.json`));
}
...
function parseArgs(argv) {
  const flags = {
    ...
    profile: 'company-os-umbrella',
    date: todayISO(),
  };
  const profileIdx = argv.indexOf('--profile');
  if (profileIdx >= 0 && argv[profileIdx + 1]) flags.profile = argv[profileIdx + 1];
...
function scaffoldRun(profile, date) {
  const workflowId = profile.id || 'company-os-umbrella';
  const runDir = repoPath('automation-runs', workflowId, date);
  ensureDir(runDir);

  const lanes = profile.lanes.map((lane) => ({
    id: lane.id,
    label: lane.label,
    codex_lane: lane.codex_lane || lane.subagent || lane.id,
    subagent: lane.subagent || null,
    status: 'pending',
...
function agentModeInstructions(profile, date, runDir, preflightResults) {
  const workflowId = profile.id || 'company-os-umbrella';
  const parallelLanes = profile.lanes.filter((l) => l.parallel_group !== 'command');
  ...
  return {
    ...
    date,
    run_dir: runDir,
    prompt: 'System/company-os-umbrella-prompt.md',
    skill: '.claude/skills/dillon-command/SKILL.md',
    parallel_lane_cap: profile.parallel_lane_cap,
    parallel_lanes: parallelLanes.map((l) => ({
      id: l.id,
      label: l.label,
      subagent: l.subagent,
      skills: l.skills,
    ...
    command_lane: commandLane
      ? {
          ...
          runs_after: commandLane.runs_after || 'intel,scout',
        }
      : null,
    instructions: [
      'Read System/company-os-umbrella-prompt.md and follow it exactly.',
      `Run: node _os/automation/bin/dillon-command.js --profile ${workflowId} --preflight`,
      `Execute Phase 1 lanes in parallel (max ${profile.parallel_lane_cap} concurrent subagents).`,
      'Phase 2 memory-consolidator runs LAST; merge all lane outputs into competitive-task-today.md and approval-board.md.',
      'Commit Daily-Briefs/, System/ operator surfaces, and automation-runs artifacts.',
...
async function main() {
  ...
  const workflowId = profile.id || flags.profile;
  const stateFile = writeRunState(workflowId, {
