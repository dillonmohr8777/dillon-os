...
function writeFixtures() {
  for (const candidate of CANDIDATES) {
    const { inspect, inspect_config, inspect_server, ...rest } = candidate;
...
function runGate() {
  ...
  for (const candidate of CANDIDATES) {
    ...
    if (candidate.inspect) {
      args.push('--inspect', '--timeout-ms', '180000');
      if (candidate.inspect_config) {
        args.push(
          '--config',
          path.resolve(dir, candidate.inspect_config),
          '--server',
          candidate.inspect_server || candidate.id,
        );
      }
    }
...
if (process.argv.includes('--gate')) {
  ...
  fs.writeFileSync('/tmp/mcp-google-ops-gate-summary.json', JSON.stringify(results, null, 2) + '\n');
  if (bad.length) {
